  ...
  return (
    <div className="min-h-screen bg-cover py-12 px-4 sm:px-6 lg:px-8" style={{ backgroundImage: `url(${backgroundImage})` }}>
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold text-center my-6 text-white">Bookmarked Articles</h1>
        {isLoading ? (
          ...
          <div className="space-y-6">
            {posts.map((post) => (
              <div key={post.id} className="bg-white bg-opacity-90 rounded-lg shadow-lg overflow-hidden transition duration-300 ease-in-out transform hover:scale-105">
                <div className="p-6">
                  ...
                  <div className="flex justify-between items-center text-sm text-gray-500">
                    <span>Published: {formatDate(post.publishedat.seconds)}</span>
                    <span>Last updated: {formatDate(post.updatedat.seconds)}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <p className="text-xl text-white text-center">No bookmarked articles found.</p>
        )}
      </div>
    </div>
  );
